...
using namespace QPipeWireAudio;
...
QPipeWireAudio::AudioObject *ListItemMenu::audioObject() const
{
    return m_audioObject.data();
}

void ListItemMenu::setAudioObject(QPipeWireAudio::AudioObject *audioObject)
{
    if (m_audioObject.data() != audioObject) {
        // TODO is Qt clever enough to catch the disconnect from base class?
        if (m_audioObject) {
            disconnect(m_audioObject, nullptr, this, nullptr);
        }

        m_audioObject = audioObject;

        if (auto *device = qobject_cast<QPipeWireAudio::Device *>(m_audioObject.data())) {
...
QPipeWireAudio::CardModel *ListItemMenu::cardModel() const
{
    return m_cardModel.data();
}

void ListItemMenu::setCardModel(QPipeWireAudio::CardModel *cardModel)
{
...
bool ListItemMenu::checkHasContent()
{
    ...
    auto *device = qobject_cast<QPipeWireAudio::Device *>(m_audioObject.data());
    ...
    if (device) {
        ...
        if (m_cardModel) {
            const int cardModelAudioObjectRole = m_cardModel->role("AudioObject");
            Q_ASSERT(cardModelAudioObjectRole != -1);

            for (int i = 0; i < m_cardModel->rowCount(); ++i) {
                const QModelIndex cardIdx = m_cardModel->index(i, 0);
                Card *card = qobject_cast<Card *>(cardIdx.data(cardModelAudioObjectRole).value<QObject *>());
...
static int getModelRole(QObject *model, const QByteArray &name)
{
    ...
    // or that AudioObjectFilterModel from QML where everything is a QVariant...
...
QMenu *ListItemMenu::createMenu()
{
    ...
    if (auto *device = qobject_cast<QPipeWireAudio::Device *>(m_audioObject.data())) {
        ...
        if (m_cardModel) {
            ...
            for (int i = 0; i < m_cardModel->rowCount(); ++i) {
                const QModelIndex cardIdx = m_cardModel->index(i, 0);
                Card *candidateCard = qobject_cast<Card *>(cardIdx.data(cardModelAudioObjectRole).value<QObject *>());
    ...
    auto *stream = qobject_cast<QPipeWireAudio::Stream *>(m_audioObject.data());
